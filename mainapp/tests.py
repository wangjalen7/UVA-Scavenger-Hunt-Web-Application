...
class ThemeCreationTestCase(TestCase):
    def setUp(self):
        self.admin_user = User.objects.create_superuser('admin_theme_creator', 'admin@example.com', 'adminpassword')

    def test_create_theme_with_embedded_tasks(self):
        ...
        theme_data = {
            ...
            'tasks': [
                {'name': 'Find a Historic Landmark', 'task': 'Locate and take a photo with a well-known historic landmark in your city.'},
                {'name': 'Nature Trail Exploration', 'task': 'Explore a nature trail and mark three different types of trees.'}
            ]
        }
        response = self.client.post(reverse('create_theme'), theme_data)
        self.assertEqual(response.status_code, 302)
        theme = Theme.objects.get(title='Adventure Theme')
        self.assertEqual(theme.description, 'Exciting adventure tasks')
        self.assertTrue(theme.tasks.filter(name='Find a Historic Landmark').exists())
        self.assertTrue(theme.tasks.filter(name='Nature Trail Exploration').exists())



class TaskCreationTestCase(TestCase):
    def setUp(self):
        self.admin_user = User.objects.create_superuser('admin_task_creator', 'admin@example.com', 'adminpassword')
        self.theme = Theme.objects.create(title='Adventure Theme', description='Exciting adventure tasks')
...
class EventFormTests(TestCase):
    ...
    def test_event_form_valid_data(self, MockEvent):
        theme = Theme.objects.create(title='Test Theme', description='Test Theme Description', tasks='')
        form = EventForm(data={
            'name': 'Test Event',
...
class EventParticipationTestCase(TestCase):
    ...
    def test_join_event(self):
        ...
        # User joins a team
        response = self.client.post(reverse('join_team', args=[self.event.id, self.team.id]))
        self.assertEqual(response.status_code, 302)
        ...
        response = self.client.post(reverse('join_team', args=[self.event.id, another_team.id]))
        self.assertNotEqual(response.status_code, 302)  # Assuming it redirects to an error page or similar
